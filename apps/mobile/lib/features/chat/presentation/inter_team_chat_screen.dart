...
Future<void> openInterTeamChat(
  ...
  // Chat inter-équipes désactivé.
}

class InterTeamChatScreen extends StatefulWidget {
  const InterTeamChatScreen({
    super.key,
  ...
  @override
  State<InterTeamChatScreen> createState() => _InterTeamChatScreenState();
}

class _InterTeamChatScreenState extends State<InterTeamChatScreen> {
  ...
  void initState() {
    ...
    socket.joinInterTeam(widget.tournamentId);
    _subs.add(socket.onInterTeamMessage.listen(_onSocketMessage));
    _subs.add(socket.onInterTeamMessageDeleted.listen(_onSocketDeleted));
    _subs.add(socket.onInterTeamChatCleared.listen(_onSocketCleared));
  }

  @override
  void dispose() {
    SocketService.instance.leaveInterTeam(widget.tournamentId);
  ...
  Future<void> _load({bool initial = false}) async {
    ...
    try {
      final messages =
          await context.read<AuthSession>().api.listInterTeamChatMessages(
                widget.tournamentId,
  ...
  Future<void> _send() async {
    ...
    try {
      final created =
          await context.read<AuthSession>().api.postInterTeamChatMessage(
                widget.tournamentId,
                text,
              );
      _input.clear();
  ...
  Future<void> _delete(Map<String, dynamic> message) async {
    ...
    try {
      await context
          .read<AuthSession>()
          .api
          .deleteInterTeamChatMessage(message['id'].toString());
    } on ApiException catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.message)));
    }
  ...
  Future<void> _clearMine() async {
    ...
    try {
      await context.read<AuthSession>().api.clearInterTeamChat(widget.tournamentId);
  ...
  Future<void> _hideChat() async {
    ...
    if (!ok || !mounted) return;
    try {
      await context.read<AuthSession>().api.hideInterTeamChat(widget.tournamentId);
      if (!mounted) return;
      widget.onLeave?.call();
    } on ApiException catch (e) {
      if (!mounted) return;
  ...
  Future<void> _clearEveryone() async {
    final ok = await confirmChatAction(
      context,
      title: 'Vider le chat inter-équipes ?',
      body: 'Tous les messages seront supprimés pour les capitaines et l’orga.',
    ...
    try {
      await context
          .read<AuthSession>()
          .api
          .clearInterTeamChatForEveryone(widget.tournamentId);
      await _load(initial: true);
    } on ApiException catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.message)));
      ...
      children: [
        ...
        Expanded(
          child: ChatThread(
            ...
            cannotSendHint: 'Réservé aux capitaines et à l’orga.',
            canDelete: (m) =>
